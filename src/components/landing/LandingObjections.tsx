    ...
    <section className="py-16 md:py-24 bg-background">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div ref={objectionsRef} className="space-y-4">
          {objections.map((item, index) => (
            <div 
              key={item.objection}
              className={cn(
                "group relative rounded-2xl border border-border bg-card p-6 md:p-8 transition-all duration-700 hover:shadow-card hover:border-primary/20",
                objectionsVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              )}
              style={{ transitionDelay: objectionsVisible ? `${index * 75}ms` : '0ms' }}
            >
        ...
        <div className={cn(
          ...
          objectionsVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
        )}>
